...
//     let cpp = eth_pairings_go::run(&data);
//     match (native, cpp) {
//         (Ok(n), Ok(c)) => {
//             if n != c {
//                 println!("Input = {}", hex::encode(&data));
...
fn run_with_op(data: &[u8]) {
    ...
    let cpp_op = eth_pairings_go::OperationType::from_u8(data[0]);
...
//     let cpp = eth_pairings_go::meter_operation(cpp_op, &data[0..]);
//     match (native, cpp) {
//         (Ok(n), Ok(c)) => {
//             if n != c {
//                 println!("Input = {}", hex::encode(&data));
